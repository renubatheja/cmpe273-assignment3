...
// Post request to uber endpoint with json 
func (c *Client) httpPostReqDo(
	endpoint string, payload string, oauth bool, out uberAPIResp,
) (error, string) {
	...
	res, err := c.sendPostRequestWithAuthorization("POST", url, payload, oauth)
	if err != nil {
		return err, ""
	}
	...
	return nil, rideResponse
}

func (c *Client) httpReqDo(
	method, endpoint string, payload uberAPIReq, oauth bool, out uberAPIResp,) error {
...
func (c *Client) sendPostRequestWithAuthorization(
	method, url string, data string, oauth bool,
) (*http.Response, error) {
	...
	fmt.Println(authStr)
	//fmt.Println("URL ", url, " Data ", data, " Method " , method)
	tr := &http.Transport{
...
func (c *Client) PostRequest(
	...
	payload := fmt.Sprintf("{ \"product_id\":  \"%s\", \"start_latitude\": %f,  \"start_longitude\":  %f, \"end_latitude\": %f,  \"end_longitude\":  %f}", productID, startLat, startLon, endLat, endLon)

	if err, response := c.httpPostReqDo(RequestEndpoint, payload, true, request); err != nil {
